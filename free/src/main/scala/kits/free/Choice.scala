...
sealed abstract class Choice[M[_]] {
  ...
  type Member[U] = kits.free.Member[Choice[M], U]
...
object Choice {
  ...
  def run[M[_]](implicit M: MonadPlus[M]) = new Run {
    type Sum[U] = Choice[M] :+: U
    type F[A] = M[A]
    def run[U, A](free: Free[Choice[M] :+: U, A]): Free[U, M[A]] =
  ...
  def zero[U, M[_]](implicit F: Member[Choice[M], U]): Free[U, Nothing] = Free(F.inject[Nothing](Zero()))

  def plus[U, M[_], A](x: Free[U, A], y: Free[U, A])(implicit F: Member[Choice[M], U]): Free[U, A] = Free[U, Boolean](F.inject(Plus())).flatMap(if (_) x else y)

  def split[U: Choice[M]#Member, M[_], A](free: Free[U, A]): Free[U, Option[(A, Free[U, A])]] = {
  ...
  def ifte[U: Choice[M]#Member, M[_], A, B](t: Free[U, A])(th: A => Free[U, B])(el: Free[U, B]): Free[U, B] =
    split(t).flatMap {
      case None => el
      case Some((a, free)) => plus(th(a), free.flatMap(th))
    }

  def once[U: Choice[M]#Member, M[_], A, B](free: Free[U, A]): Free[U, A] =
    split(free).flatMap {
      case None => zero
