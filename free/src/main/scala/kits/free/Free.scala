...
object Free {
  ...
  def handleRelay[F <: { type T }, U <: Union, A, B, S](free: Free[F :+: U, A], state: S)(f: (A, S) => Either[(Free[F :+: U, A], S), Free[U, B]])(g: (F, S) => (Any => Free[F :+: U, A]) => Either[(Free[F :+: U, A], S), Free[U, B]]): Free[U, B] =
    free match {
      case Pure(a) =>
        f(a, state) match {
